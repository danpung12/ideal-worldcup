...
const CANDIDATES = [
  { id: 1, name: '카리나', img: 'https://placehold.co/400x400/FFB6C1/white?text=Karina' },
  { id: 2, name: '윈터', img: 'https://placehold.co/400x400/87CEEB/white?text=Winter' },
  { id: 3, name: '하니', img: 'https://placehold.co/400x400/98FB98/white?text=Hanni' },
  { id: 4, name: '민지', img: 'https://placehold.co/400x400/DDA0DD/white?text=Minji' },
];
...
io.on('connection', (socket)=>{ // 연결 시
	console.log('접속:', socket.id); // 접속:id 띄워 줌
	
    socket.on('join_room', (roomName) =>{ // 방 입장
        socket.join(roomName);
        console.log(`유저 (${socket.id})가 [${roomName}] 방에 입장함`);
    });

	socket.on('chat_msg', (data) =>{ // 메세지 전송
    ...
    socket.on('vote', ({room, winnerId}) => {
        ...
        if (game.nowIdx > game.candidates.length){
            ...
            if (game.winner.length == 1){
                console.log(`우승은 ${game.winner} 입니다.`)
                return io.to(room).emit('game_over', game.winner[0]);
            } 

            // 진출자로 후보 목록을 교체
            game.candidates = game.winner;
            game.winners = [];
            game.nowIdx = 0;

        }
        // 다음 대결 안내
        io.to(room).emit('round_start', game.candidates.slice(game.nowIdx, game.nowIdx+2));;
